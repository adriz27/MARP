...
protected:
   ...
   struct TreeNode {
      ...
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int posicion = 1) : elem(e), iz(i), dr(d), altura(alt), pos(posicion) {}
   };
...
public:
   ...
   T const& kesimo(int k) const {
	   if (k < 1 || k > nelems) { throw std::invalid_argument(""); }
...
protected:
	...
	T const& kesimo(Link a, int k) const {
		if (k == a->pos) { return a->elem; }
		else if (k < a->pos) { return kesimo(a->iz, k); }
		else if(a->dr != nullptr){ return kesimo(a->dr, k - a->pos); }
   ...
   void rotaDer(Link & r2) {
	  ...
	  if (r2->iz != nullptr) { r2->pos = r2->iz->pos + 1; }
	  else { r2->pos = 1; }
      r2 = r1;
   }
   ...
   void rotaIzq(Link & r1) {
      ...
      r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
      r2->altura = std::max(altura(r2->iz), altura(r2->dr)) + 1;
	  if (r1->dr != nullptr) { r2->pos = r1->pos + 1 + r1->dr->pos; }
	  else { r2->pos = r1->pos + 1; }
      r1 = r2;
   }

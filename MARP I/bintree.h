...
public:
	...
	bool avl() {
		int h = 0, min = -1000000000, max = 1000000000;
		return avl(raiz, h, min, max);
	}
...
protected:
	...
	bool avl(Link a, int& h, int& min, int& max) {
		if (a == nullptr) { return true; }
		if (a->elem >= max || a->elem <= min) { return false; }
		int h1 = 0, h2 = 0, min2 = a->elem, max1 = a->elem, min1 = min, max2 = max;
		if (!avl(a->left, h1, min1, max1)) { return false; }
		if (!avl(a->right, h2, min2, max2)) { return false; }
